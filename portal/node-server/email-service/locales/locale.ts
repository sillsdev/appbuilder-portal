...
type DeepKeys<T extends Record<string, unknown>> = {
    [K in keyof T]: K extends string ?
        T[K] extends Record<string, unknown> ? `${K}.${DeepKeys<T[K]>}` : K
        : never;
}[keyof T];
export function translate<T extends DeepKeys<typeof en>>(locale: 'en' | 'es-419' | 'fr-FR', key: T, options?: Record<string, string>): string
