import { Interactor, interactor, clickable, findAll, isPresent, text } from '@bigtest/interactor';
...
class OrgSwitcher {
  ...
  chooseOrganization(this: Interactor, orgText: string) {
    return this.when<HTMLElement>(() => {
      const el = this.$$('[data-test-org-select-item]').find((item) =>
        item.innerText.includes(orgText)
      );
...
export default new (OrgSwitcherInteractor as any)(
  '[data-test-org-switcher]'
) as TOrgSwitcherInteractor;
