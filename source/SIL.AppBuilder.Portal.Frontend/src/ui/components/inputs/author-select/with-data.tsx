...
export function withData(WrappedComponent) {
  function DataWrapper({
    ...
    const { 
      ...
      users: [(q) => q.findRecords('user'), buildOptions({include: ['group-memberships.group', 'organization-memberships.organization','user-roles']})], 
    ...
    const authorRoles = (userRoles || []).filter((r) => r && scopeToOrganization && isRelatedRecord(r, scopeToOrganization) && isRelatedRecord(r, authorRole));
    const authorUserIds = (authorRoles || []).map((r) => r && relationshipFor(r, "user").data['id']);

    const existingAuthors = (authors || []).filter((a) => a && project && isRelatedRecord(a, project));
    const existingAuthorUserIds = existingAuthors.map((a) => a && relationshipFor(a, "user").data['id'] );


    // remove users who _can't_ be assigned to this project as authors
    const filtered = (users || []).filter((user) => authorUserIds.includes(user.id)).filter((user) => {
    ...
    return (
      <WrappedComponent
        {...{ ...otherProps, selected, users: filtered }}
      />
    );
  }
        ...
        (q) => q.findRecords(USER),
        {
          ...
          sources: {
            remote: {
              ...
              include: ['group-memberships.group', 'organization-memberships.organization', 'user-roles'],
            },
          },
        },
      ],
