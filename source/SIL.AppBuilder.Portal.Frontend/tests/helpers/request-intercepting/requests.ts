...
export const mockGet = (server) => (
  ...
  server.namespace('/api', async () => {
    await server
      .get(path)
      .intercept(respondWithJsonApi(status, jsonapiPayload, bindCustomize(server, customize)));
  });
};

export const mockPatch = (server) => (
  status: number,
  path: string,
  jsonapiPayload: {},
  customize?: CustomizeCallback
...
export const mockPost = (server) => async (
  status: number,
  path: string,
  jsonapiPayload: {},
  customize?: CustomizeCallback
...
export const mockDelete = (server) => (
  status: number,
  path: string,
  customize?: CustomizeCallback
) => {
