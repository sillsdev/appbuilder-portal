...
export function getThemeStyle(themeColor: string | null | undefined) {
  ...
  const int = Number.parseInt(primaryHex.slice(1), 16);
  ...
  const darkContrast =
    (Math.max(luminance, darkContentLuminance) + 0.05) /
    (Math.min(luminance, darkContentLuminance) + 0.05);
  const primary = `hsl(${(hue * 360).toFixed(1)} ${(saturation * 100).toFixed(1)}% ${(lightness * 100).toFixed(1)}%)`;
  // Pick readable button/header text for the manifest color.
  const primaryContent = darkContrast >= lightContrast ? '#0f172a' : '#ffffff';

  return `--color-primary: ${primary}; --color-primary-content: ${primaryContent};`;
}
