...
This is placeholder content to demonstrate layout only.`
  ...
  let selectedLocale = $state(locales[0]);
...
<div class="min-h-screen flex items-center justify-center bg-base-200 font-sans p-4" style="background-color: var(--outer-bg)">
        ...
        <div class="p-8">
            {#if !isVerified}
                ...
                <div class="mt-6 text-center text-sm text-base-content/60">
                    <p>Didn't receive the code? <button class="link link-primary font-bold no-underline hover:underline bg-transparent border-none p-0 cursor-pointer">Resend Email</button></p>
                </div>
            {:else}
                <div class="text-center flex flex-col gap-4">
                    <p class="text-lg font-bold text-base-content">Verification Complete</p>
                    <p class="text-base-content/70 text-[0.95rem]">The process has been completed and your information was removed.</p>
                    <button class="btn btn-outline btn-primary mt-4 rounded-xl" onclick={() => window.location.reload()}>
                        Done
                    </button>
                </div>
            {/if}
