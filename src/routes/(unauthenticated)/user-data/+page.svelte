<script lang="ts">
  ...
  import { AlternateCodes, UDMFlags } from '$lib/udm';
  import { getLocale, locales, setLocale } from '$lib/udm/paraglide/runtime';
  ...
  {#each locales.toSorted( (a, b) => byString(displayNames.get(a)?.display, displayNames.get(b)?.display, current) ) as locale}
    ...
    >
      <div class="flex flex-col py-1 w-full items-start">
        <span>
          <IconContainer icon={getFlagIcon(locale, UDMFlags)} width={24} />
